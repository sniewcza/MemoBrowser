...
export const MemoListView: FC<Props> = props => {
  ...
  React.useLayoutEffect(() => {
    props.navigation.setOptions({
      headerRight: () => (
        <Animated.View
          style={[styles.headerButton, {transform: [{rotateZ: rotation}]}]}>
          <IconButton
  ...
  const handleMemoItemPress = useCallback(
    (id: string) => {
  ...
  const handleMemoItemCheckChange = useCallback((id: string) => {
    ...
    if (index !== -1) {
      setMemosToDelete([
        ...memosToDelete.slice(0, index),
        ...memosToDelete.slice(index + 1),
      ]);
    } else {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Loader
        isLoading={isLocked === undefined || memos === undefined}
        ...
        />
        <Animated.View
          style={[styles.actionButton, {opacity, transform: [{scale}]}]}>
          <ActionButton
        ...
        <Animated.View
          style={[
            styles.menuBar,
            {transform: [{translateY: menuBarTranslateY}]},
          ]}>
          <DeletionBottomBar
            onCancelPress={cancelDeletionMode}
            onDeletePress={deleteMemos}
          />
        </Animated.View>
      </Loader>
    </SafeAreaView>
